...
def init_db():
    ...
    test_user = User(username="roy", email="roy.wiggins@nyumc.org", password=pwd_context.hash("roy"),roles=[admin_role,submit_role])
    db.session.add(test_user)
    tobias = User(username="tobias", email='kaitobias.block@nyumc.org', password=pwd_context.hash("tobias"),roles=[admin_role,submit_role])
    db.session.add(tobias)


    servers = [YarraServer(name='AGR1',username='yarra',path='xdglpdcdap001.nyumc.org', roles=[submit_role]), 
                YarraServer(name='YarraAda',username='yarra',path='xdglpdcdap002.nyumc.org',roles=[admin_role])]

    for s in servers:
        s.password = 'melbourne'
        s.update_modes()
        db.session.add(s)
